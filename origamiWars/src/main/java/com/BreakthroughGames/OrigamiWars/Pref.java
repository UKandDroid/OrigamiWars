		...
		switch(vType)
			...
			case GAME_OVER:
				...
				if(pref.getLong(PREF_STORY_SCORE, 0) < Game.scoreBoard.getScore())
					{
					pref.edit().putLong(PREF_STORY_SCORE, Game.scoreBoard.getScore()).commit();	
					Adventure.events.dispatch(Events.HIGHEST_SCORE);
					bNoEvent= false;
					}
				if(pref.getLong(PREF_TOP_WINS, 0) < (PowerUps.iTotalWins + PowerUps.iLevelWins) )
					{
					pref.edit().putLong(PREF_TOP_WINS, (PowerUps.iTotalWins + PowerUps.iLevelWins)).commit();	
				...
				if(pref.getLong(PREF_TOP_LOSES, 0) < (PowerUps.iTotalLoses + PowerUps.iLevelLoses) )
					{
					pref.edit().putLong(PREF_TOP_LOSES, (PowerUps.iTotalLoses + PowerUps.iLevelLoses)).commit();	
