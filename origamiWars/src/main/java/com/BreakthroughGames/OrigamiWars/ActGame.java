...
public class ActGame extends Activity 					
{
	...
	{	Log.d("MethodCall", "ActGame::OnCreate() Called");
		...
		iBackPressed = 0;
		if(Game.iMode == Values.START_ERROR) finish();										// Go to back to Main activity as app memory has been reclaimed by OS
	...
	************************************************************************************************************************/
	...
	public boolean onKeyUp(int vKey, KeyEvent vEvent) {
		...
		if(vKey == KeyEvent.KEYCODE_BACK)
			{
			if(iBackPressed == 0)
				{Screen.iMenu = Screen.MENU_PAUSE; ActMainMenu.handler.sendEmptyMessage(ActMainMenu.BACK_PRESSED);}
			else	
