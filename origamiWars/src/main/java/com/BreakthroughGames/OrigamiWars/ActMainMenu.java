...
public class ActMainMenu extends Activity
{
	...
	public void onCreate(Bundle sis) {
		...
		if(!Pref.bFB_Like && bShowOnce)	{												// If Game hasn't been liked on FaceBook, Show Toast to like it
			bShowOnce = false;
			new Handler().postDelayed(new Thread() {@Override public void run() { facebook.setImageResource(R.drawable.btn_facebook_like); facebook.startAnimation(anim); }}, 4000);
			new Handler().postDelayed(new Thread() {@Override public void run() { facebook.setImageResource(R.drawable.btn_facebook); }}, 7500);
		}
				...
				public void handleMessage(Message msg) {
					switch(msg.what) {
						...
						case POST_SUCCESSFULL:
							new Handler().post(new Thread() {@Override public void run() { facebook.setImageResource(R.drawable.btn_facebook_posted); facebook.startAnimation(anim); }});
							new Handler().postDelayed(new Thread() {@Override public void run() { facebook.setImageResource(R.drawable.btn_facebook); }}, 3000);
							break;
						case POST_ERROR:
							new Handler().post(new Thread() {@Override public void run() { facebook.setImageResource(R.drawable.btn_facebook_post_error); facebook.startAnimation(anim); }});
							new Handler().postDelayed(new Thread() {@Override public void run() { facebook.setImageResource(R.drawable.btn_facebook); }}, 3000);
							break;
 ...
 ************************************************************************************************************************/
		// TWITTER Button
		twitter.setOnClickListener(new OnClickListener(){@Override
		...
		// FACE BOOK Button
		facebook.setOnClickListener(new OnClickListener(){@Override
		public void onClick(View v) {
		...
		// SURVIVAL button
		arcade.setOnClickListener(new OnClickListener(){@Override
		public void onClick(View v) {
			GLWrapper.testMethod();
			if(Events.vibrate != null)	Events.vibrate.vibrate(40);							// Give feedback to user
		...
		contin.setOnClickListener(new OnClickListener(){@Override
		public void onClick(View v) {
			if(Events.vibrate != null)	Events.vibrate.vibrate(40);							// Give feedback to user
			Screen.iMenu = Screen.MENU_OFF;
			Intent game = new Intent(getApplicationContext(), ActGame.class);
	...
	@Override
	public void onBackPressed() {	exitGame();		} 								// If back Button is pressed, Exit the Game
