...
public class Arcade extends Game
{
	...
	Arcade(GL10 glRef)
	{
		...
		for(int i=0; i < MAX_OBJECTS; i++) object[i] = new Object();					// Initialize Objects
		for(int i=0; i < Weapon.MAX_SHOTS; i++)	fire[i] = new Weapon();					// Initialize weapons + ShockWave		
	}
	...
	{Values.log("MethodCall", "Game::reset() Called");
		...
		for(int i=0; i < MAX_OBJECTS; i++) object[i].bEnable = false;					// Initialize Objects
	}
	/************************************************************************************************************************
