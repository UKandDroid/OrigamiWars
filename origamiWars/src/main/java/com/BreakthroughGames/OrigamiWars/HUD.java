			...
			for(int i = 0; i < 4; i++)									// Go through all the weapons, 
				if(Weapon.arRounds[i] > 0)								// Draw those Weapons which got shot 
					...
					if(i == Weapon.curWeapon)
						{
						if(i != 0) iconShift += 0.30f;					// Shift Icon, so it dont overlap on previous Icon, if its not first
	...
	protected static int levelEndStats()
	{
		...
		switch(iCounter)
			...
			default:
				...
				if(temCounter > 5)
					{
					if(lvlStatsSpeed == 8)	Screen.bPause = true;		// if user hasn't touched the screen, pause the screen
