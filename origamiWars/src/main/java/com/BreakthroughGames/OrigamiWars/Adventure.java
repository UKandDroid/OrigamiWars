...
public class Adventure extends Game {
    ...
    Adventure(GL10 glRef) {
        ...
        for (int i = 0; i < MAX_OBJECTS; i++)
            object[i] = new Object();                    // Initilize Objects
        for (int i = 0; i < Weapon.MAX_SHOTS; i++)
            fire[i] = new Weapon();                    // Initilize weapons + ShockWave
    }

    public void initObjects() {
    }                                                        // Methods Overriden by children
    ...
    protected static void calcLevelStats() {
        ...
        if (bSecond) {
            ...
            if (ran.nextFloat() < lvlObstFreq)
                calcObstacles(Values.PATH_STRAIGHT_SLOW, ran.nextInt(cloudType + 1));    // Set obstacles for the level, based on obstacle difficulty
    ...
    protected static void createEnemy(Object obj) {
        ...
        for (tType = Values.ENEMY_LEAF; tType < Values.ENEMY_END; tType++)                        // Get the first type of enemy thats still left, Checking Distance array, starting from leaf
            if (arDistance[tType] != null)
                break;                                                // if Distance[EnemyType] == null, that means all enemies of this type are created, move to next type
